...
function getMethod(operation: string): IHttpRequestMethods {
	switch (operation) {
		case 'create':
			return 'POST';
		case 'update':
...
function buildNamespaceRequest(
	...
	switch (operation) {
		case 'create': {
			const name = ctx.getNodeParameter('namespaceName', index) as string;
		...
		case 'get': {
			const namespaceId = ctx.getNodeParameter('namespaceId', index) as string;
		...
		case 'list': {
			const pagination = ctx.getNodeParameter('pagination', index, {}) as IDataObject;
...
function buildBucketRequest(
	...
	switch (operation) {
		case 'create': {
			const name = ctx.getNodeParameter('bucketName', index) as string;
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
			Object.assign(body, { name }, additionalFields);
			if (additionalFields.schema) {
				body.schema = JSON.parse(additionalFields.schema as string);
			}
			return '/v1/buckets';
		}
		case 'get': {
			const bucketId = ctx.getNodeParameter('bucketId', index) as string;
...
function buildCollectionRequest(
	...
	switch (operation) {
		case 'create': {
			const name = ctx.getNodeParameter('collectionName', index) as string;
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
			Object.assign(body, { name }, additionalFields);
			if (additionalFields.extractors) {
				body.extractors = JSON.parse(additionalFields.extractors as string);
			}
			return '/v1/collections';
		}
		case 'get': {
			const collectionId = ctx.getNodeParameter('collectionId', index) as string;
...
function buildDocumentRequest(
	...
	switch (operation) {
		case 'create': {
			const documentData = ctx.getNodeParameter('documentData', index) as string;
			Object.assign(body, JSON.parse(documentData));
			return `/v1/collections/${collectionId}/documents`;
		}
		case 'get': {
			const documentId = ctx.getNodeParameter('documentId', index) as string;
			return `/v1/collections/${collectionId}/documents/${documentId}`;
		}
		case 'update': {
			const documentId = ctx.getNodeParameter('documentId', index) as string;
			const updateData = ctx.getNodeParameter('updateData', index) as string;
...
function buildRetrieverRequest(
	...
	switch (operation) {
		case 'create': {
			const name = ctx.getNodeParameter('retrieverName', index) as string;
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
			Object.assign(body, { name }, additionalFields);
			if (additionalFields.stages) {
				body.stages = JSON.parse(additionalFields.stages as string);
			}
...
function buildUploadRequest(
	...
	switch (operation) {
		case 'create': {
			const filename = ctx.getNodeParameter('filename', index) as string;
			const contentType = ctx.getNodeParameter('contentType', index) as string;
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
		...
		case 'get': {
			const uploadId = ctx.getNodeParameter('uploadId', index) as string;
...
function buildTaskRequest(
	...
	switch (operation) {
		case 'get': {
			const taskId = ctx.getNodeParameter('taskId', index) as string;
...
function buildInferenceBody(ctx: IExecuteFunctions, index: number): IDataObject {
	const model = ctx.getNodeParameter('model', index) as string;
	const input = ctx.getNodeParameter('input', index) as string;
	const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
	...
	if (additionalFields.parameters) {
		body.parameters = JSON.parse(additionalFields.parameters as string);
	}

	return body;
}

function buildTaxonomyRequest(
	ctx: IExecuteFunctions,
	index: number,
	operation: string,
	body: IDataObject
	...
	switch (operation) {
		case 'create': {
			const name = ctx.getNodeParameter('taxonomyName', index) as string;
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
			Object.assign(body, { name }, additionalFields);
			if (additionalFields.categories) {
				body.categories = JSON.parse(additionalFields.categories as string);
			}
			return '/v1/taxonomies';
		}
		case 'get': {
			const taxonomyId = ctx.getNodeParameter('taxonomyId', index) as string;
...
function buildClusterRequest(
	...
	switch (operation) {
		case 'create': {
			const name = ctx.getNodeParameter('clusterName', index) as string;
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
			Object.assign(body, { name }, additionalFields);
			if (additionalFields.configuration) {
				body.configuration = JSON.parse(additionalFields.configuration as string);
			}
			return '/v1/clusters';
		}
		case 'get': {
			const clusterId = ctx.getNodeParameter('clusterId', index) as string;
...
function buildWebhookRequest(
	...
	switch (operation) {
		case 'create': {
			const url = ctx.getNodeParameter('webhookUrl', index) as string;
			const events = ctx.getNodeParameter('events', index) as string[];
			const additionalFields = ctx.getNodeParameter('additionalFields', index) as IDataObject;
			Object.assign(body, { url, events }, additionalFields);
			return '/v1/organizations/webhooks';
		}
		case 'get': {
			const webhookId = ctx.getNodeParameter('webhookId', index) as string;
...
export class Mixpeek implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const baseUrl = (credentials.baseUrl as string) || 'https://api.mixpeek.com';
